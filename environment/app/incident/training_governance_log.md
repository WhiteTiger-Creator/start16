...
> **Governance decision (2026-05-09 - #ML-6174)** Yusuf: Recovered shape, final. The rebuilt registry is a JSON array ascending by step, then rank, then kind, then entry id. Each row carries the eight declared fields -- the migrator's own bookkeeping (`seq`, the CHANGE kind, and `posted_by`) never survives the replay. The change kind is the `amend`/`invalidate`/`reinstate` label a journal row carries and is a different field from the registry entry's own `kind`, which names the shard as `model` or `optimizer` and is one of the eight the rebuilt row keeps.
